...
| `CASE-4` | Authority is granted for one exact work unit or protected-area exception, but the proposed action also mutates another artifact, activates a protected future capability, or operates on an external Target | `Mission/AGENTS.md` §6 Protected Areas; `Mission/AGENTS.md` §7 Execution Authorization Boundaries (`Scope`, `No expansion`, `Exactly one work unit`); `ProjectStatus.md` §7 Protected Areas | **Observed result: `Blocker`.** Reject the proposed action because permission does not transfer across artifacts, work units, capabilities, protected-area exceptions, or Target boundaries. |
