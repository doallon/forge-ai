...
| Traceability ID | FORGE-AI.TEMPLATES.WORKFLOW.DECISION |
| Scope | Reusable template for operational decision records in the Workflow template family |
| Out of Scope | Architecture decisions, governance approval, certification decisions, or ProjectStatus updates |
...
| Produces | Operational decision record |
...
This template defines the required structure for producing a governed operational decision record. It is an output contract only: it consumes existing workflow, command, and orchestration authorities without redefining them.
...
Use this template for workflow, process, execution, tooling, or governance-routing decisions. Use `docs/AI/Templates/Architecture/ADRTemplate.md` for architecture decisions.

| Option | Summary | Tradeoff | Outcome |
|:---|:---|:---|:---|
| `{{OPTION}}` | `{{SUMMARY}}` | `{{TRADEOFF}}` | Selected / Rejected |

## 7. Allowed Content

- Operational decision context, options, decision, rationale, workflow/command impacts, preserved constraints, follow-up actions, and review/expiry status.

## 8. Forbidden Content

- Architecture decisions, authority promotion, certification outcomes, ProjectStatus changes, or duplicate decision rights.

## 9. Validation Requirements

- Confirm STD-010 metadata is complete.
- Confirm the decision is operational and not architectural.
- Confirm constraints preserve governing workflow, command, and orchestration authorities.
- Confirm follow-up actions do not expand scope without approval.
...
- [ ] Review or expiry status defined
