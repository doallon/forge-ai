...
This template defines the required structure for producing a governed review document. It is an output contract only: it consumes existing workflow, command, and orchestration authorities without redefining them.
...
|:---|:---|:---|:---|:---|:---|:---|
| `{{FINDING_ID}}` | INFO / WARNING / BLOCKER | `{{AREA}}` | `{{FINDING}}` | `{{EVIDENCE}}` | `{{RECOMMENDATION}}` | Open / Closed |

Allowed verdicts: `PASS`, `PASS WITH OBSERVATIONS`, `REQUIRES FOLLOW-UP`, or `FAILED`.

## 7. Allowed Content

- Review purpose, scope, required reading, validation evidence, review areas, findings, risks, verdict, certification readiness, project-state impact statement, and recommendations.

## 8. Forbidden Content

- Implementation changes, validation substitution, certification claims, direct ProjectStatus updates, or recommendations that expand scope without authority.

## 9. Validation Requirements

- Confirm STD-010 metadata is complete.
- Confirm required reading and validation evidence are listed.
- Confirm findings are evidence-backed and severity-labeled.
- Confirm verdict and project-state impact are bounded by governance authority.
