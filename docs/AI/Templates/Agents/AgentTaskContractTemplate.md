...
| Field | Value |
|:---|:---|
| Identifier | `FORGE-TPL-AGENTS-TASK-CONTRACT` |
| Title | Agent Task Contract Template |
| Version | `2.0.0-draft` |
| Status | Draft |
...
| Traceability ID | `FORGE-AI.V2.TEMPLATES.AGENTS.TASK-CONTRACT` |
| Scope | Defines the reusable structure for a bounded executable task for an agent, including objective, authority chain, scope, inputs, allowed actions, forbidden actions, deliverables, validation requirements, completion reporting, blockers, and checklist. |
...
| Produces | Reusable agent task contract artifact structure |
...
Defines the reusable structure for a bounded executable task for an agent, including objective, authority chain, scope, inputs, allowed actions, forbidden actions, deliverables, validation requirements, completion reporting, blockers, and checklist.
...
Use this template to produce a `Agent Task Contract` artifact when the active task explicitly needs that artifact type.
...
```
Default produced-document classification is `Agent Artifact` and default document type is `Agent Task Contract`.
...
| `{{RELATED_PHASE_STAGE_CAPABILITY}}` | TEXT | Body content | Yes | Author-provided |
| `{{TASK_OBJECTIVE}}` | TEXT | Body content | Yes | Author-provided |
| `{{AUTHORITY_CHAIN}}` | TEXT | Body content | Yes | Author-provided |
...
| `{{ALLOWED_ACTIONS}}` | LIST | Body content | Yes | Author-provided |
| `{{FORBIDDEN_ACTIONS}}` | LIST | Body content | Yes | Author-provided |
| `{{DELIVERABLES}}` | TABLE | Body content | Yes | Author-provided |
| `{{VALIDATION_REQUIREMENTS}}` | LIST | Body content | Yes | Author-provided |
| `{{COMPLETION_REPORT_REQUIREMENTS}}` | LIST | Body content | Yes | Author-provided |
