...
| Produces | Reusable agent role artifact structure |
...
Defines the reusable structure for one governed agent role, including responsibilities, allowed work, forbidden work, required inputs, outputs, validation obligations, and escalation conditions.
...
Use this template to produce a `Agent Role` artifact when the active task explicitly needs that artifact type.
...
```
Default produced-document classification is `Agent Artifact` and default document type is `Agent Role`.
...
| `{{AGENT_ROLE_ID}}` | IDENTIFIER | Body content | Yes | Author-provided |
| `{{ROLE_SUMMARY}}` | TABLE | Body content | Yes | Author-provided |
| `{{RESPONSIBILITIES}}` | LIST | Body content | Yes | Author-provided |
| `{{ALLOWED_WORK}}` | LIST | Body content | Yes | Author-provided |
| `{{FORBIDDEN_WORK}}` | LIST | Body content | Yes | Author-provided |
| `{{REQUIRED_INPUTS}}` | TABLE | Body content | Yes | Author-provided |
| `{{OUTPUTS}}` | TABLE | Body content | Yes | Author-provided |
| `{{ESCALATION_CONDITIONS}}` | LIST | Body content | Yes | Author-provided |
