...
| Produces | Reusable agent handoff artifact structure |
...
Defines the reusable structure for transferring scoped work, context, findings, evidence, risks, required reading, artifacts, and receiver instructions from one agent or role to another.
...
Use this template to produce a `Agent Handoff` artifact when the active task explicitly needs that artifact type.
...
```
Default produced-document classification is `Agent Artifact` and default document type is `Agent Handoff`.
...
| `{{FROM_AGENT_OR_ROLE}}` | TEXT | Body content | Yes | Author-provided |
| `{{TO_AGENT_OR_ROLE}}` | TEXT | Body content | Yes | Author-provided |
| `{{TASK_ID}}` | IDENTIFIER | Body content | Yes | Author-provided |
| `{{HANDOFF_CONTEXT}}` | TEXT | Body content | Yes | Author-provided |
| `{{WORK_COMPLETED}}` | LIST | Body content | Yes | Author-provided |
| `{{WORK_REMAINING}}` | LIST | Body content | Yes | Author-provided |
| `{{REQUIRED_READING}}` | TABLE | Body content | Yes | Author-provided |
| `{{ARTIFACTS}}` | TABLE | Body content | Yes | Author-provided |
| `{{VALIDATION_EVIDENCE}}` | TABLE | Body content | Yes | Author-provided |
| `{{RISKS_OR_BLOCKERS}}` | LIST | Body content | Yes | Author-provided |
| `{{RECEIVER_INSTRUCTIONS}}` | LIST | Body content | Yes | Author-provided |
| `{{HANDOFF_CHECKLIST}}` | LIST | Body content | Yes | Author-provided |
---
...
## 4. Required Reading for Receiver
