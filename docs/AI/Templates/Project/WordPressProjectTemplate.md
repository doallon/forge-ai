...
| Produces | WordPress Project document structure and placeholder manifest |
...
The WordPress Project Template extends the Generic Project Template for WordPress plugins, themes, or hosted applications that adopt the AI Framework while preserving WordPress as an adapter or host rather than automatic domain truth.
...
| `{{DOCUMENT_TITLE}}` | STRING | Title | Yes | WordPress Project |
...
| `{{DOCUMENT_TYPE}}` | STRING | Document Type | Yes | WordPress Project Specification |
...
| `{{PRODUCES}}` | LIST | Produces | Yes | WordPress Project document structure and placeholder manifest |
...
| `{{PROJECT_TYPE}}` | STRING | Not applicable | Yes | WordPress |
| `{{TARGET_PLATFORM}}` | STRING | Not applicable | Yes | WordPress plugin/theme/application |
| `{{HOST_FRAMEWORK}}` | STRING | Not applicable | Yes | WordPress |
| `{{RUNTIME_CONTEXT}}` | STRING | Not applicable | Yes | WordPress / PHP runtime |
...
| Target Platform | WordPress plugin/theme/application |
| Host Framework | WordPress |
| Runtime Context | WordPress / PHP runtime |
| Language Stack | PHP |
