...
| Field | Value |
|:---|:---|
| Identifier | `FORGE-TPL-PROJECT-CLI` |
| Title | CLI Project Template |
| Version | `0.2.0-draft` |
| Status | Draft |
...
| Traceability ID | `FORGE-AI.TEMPLATES.PROJECT.CLI` |
| Scope | Reusable output contract for cli project adoption documentation. |
...
| Produces | CLI Project document structure and placeholder manifest |
...
The CLI Project Template extends the Generic Project Template for command-line applications, automation tools, developer utilities, and local agents. A CLI is an execution interface and must not become architecture authority.
...
| `{{DOCUMENT_TITLE}}` | STRING | Title | Yes | CLI Project |
...
| `{{DOCUMENT_TYPE}}` | STRING | Document Type | Yes | CLI Project Specification |
...
| `{{PRODUCES}}` | LIST | Produces | Yes | CLI Project document structure and placeholder manifest |
...
| `{{PROJECT_TYPE}}` | STRING | Not applicable | Yes | CLI |
| `{{TARGET_PLATFORM}}` | STRING | Not applicable | Yes | Command line |
| `{{HOST_FRAMEWORK}}` | STRING | Not applicable | Yes | CLI host |
| `{{RUNTIME_CONTEXT}}` | STRING | Not applicable | Yes | Local process / shell boundary |
...
| Host Framework | CLI host |
| Runtime Context | Local process / shell boundary |
...
CLI commands parse user intent and call application services. They must not own domain rules, planning truth, validation authority, runtime authority, or project state.
...
| `src/infrastructure/` | Filesystem, process execution, external tool adapters. |
