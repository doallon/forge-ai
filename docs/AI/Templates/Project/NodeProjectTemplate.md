...
| Field | Value |
|:---|:---|
| Identifier | `FORGE-TPL-PROJECT-NODE` |
...
| Traceability ID | `FORGE-AI.TEMPLATES.PROJECT.NODE` |
| Scope | Reusable output contract for node project adoption documentation. |
...
| Produces | Node Project document structure and placeholder manifest |
...
The Node Project Template extends the Generic Project Template for Node.js services, APIs, packages, workers, and tooling systems. Node is a runtime host and must not redefine framework or project architecture.
...
| `{{DOCUMENT_TITLE}}` | STRING | Title | Yes | Node Project |
...
| `{{DOCUMENT_TYPE}}` | STRING | Document Type | Yes | Node Project Specification |
...
| `{{PRODUCES}}` | LIST | Produces | Yes | Node Project document structure and placeholder manifest |
...
| `{{PROJECT_TYPE}}` | STRING | Not applicable | Yes | Node |
| `{{TARGET_PLATFORM}}` | STRING | Not applicable | Yes | Node.js |
| `{{HOST_FRAMEWORK}}` | STRING | Not applicable | Yes | Node.js host / project-selected framework |
| `{{RUNTIME_CONTEXT}}` | STRING | Not applicable | Yes | Node.js runtime |
...
| Host Framework | Node.js host / project-selected framework |
| Runtime Context | Node.js runtime |
...
Node routes, package scripts, queues, workers, and adapters are implementation mechanisms. Domain and application contracts should remain independent of the runtime framework where practical.

| Area | Responsibility |
|:---|:---|
| `src/core/` | Bootstrap and shared wiring documentation. |
| `src/domain/` | Domain rules and contracts. |
| `src/application/` | Use cases and handlers. |
| `src/api/` | Transport route documentation. |
| `src/infrastructure/` | Persistence, queues, cache, and external adapters. |
