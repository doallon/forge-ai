...
| Field | Value |
|:---|:---|
| Identifier | `FORGE-TPL-PROJECT-REACT` |
...
| Traceability ID | `FORGE-AI.TEMPLATES.PROJECT.REACT` |
| Scope | Reusable output contract for react project adoption documentation. |
...
| Produces | React Project document structure and placeholder manifest |
...
The React Project Template extends the Generic Project Template for frontend applications, UI shells, design systems, and React packages. React is a presentation/runtime adapter and must not own domain truth unless governance explicitly defines that boundary.
...
| `{{DOCUMENT_TITLE}}` | STRING | Title | Yes | React Project |
...
| `{{DOCUMENT_TYPE}}` | STRING | Document Type | Yes | React Project Specification |
...
| `{{PRODUCES}}` | LIST | Produces | Yes | React Project document structure and placeholder manifest |
...
| `{{PROJECT_TYPE}}` | STRING | Not applicable | Yes | React |
| `{{TARGET_PLATFORM}}` | STRING | Not applicable | Yes | Frontend / UI |
| `{{HOST_FRAMEWORK}}` | STRING | Not applicable | Yes | React |
| `{{RUNTIME_CONTEXT}}` | STRING | Not applicable | Yes | Browser / React host |
| `{{LANGUAGE_STACK}}` | STRING | Not applicable | Yes | JavaScript / TypeScript |
...
| Project Type | React |
| Target Platform | Frontend / UI |
| Host Framework | React |
| Runtime Context | Browser / React host |
...
React components, hooks, stores, routes, and UI state are presentation or application mechanisms. They must not silently become canonical business truth when domain or schema contracts exist elsewhere.

| Area | Responsibility |
|:---|:---|
| `src/domain/` | UI-domain contracts and pure rules when applicable. |
| `src/application/` | Use cases, orchestration, and state adapters. |
| `src/infrastructure/` | API clients, storage, analytics, and integrations. |
| `src/presentation/` | Screens, layouts, components, and visual composition. |
