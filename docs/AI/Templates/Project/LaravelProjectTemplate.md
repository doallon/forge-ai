...
| Field | Value |
|:---|:---|
| Identifier | `FORGE-TPL-PROJECT-LARAVEL` |
| Title | Laravel Project Template |
| Version | `0.2.0-draft` |
| Status | Draft |
...
| Traceability ID | `FORGE-AI.TEMPLATES.PROJECT.LARAVEL` |
| Scope | Reusable output contract for laravel project adoption documentation. |
...
| Produces | Laravel Project document structure and placeholder manifest |
...
The Laravel Project Template extends the Generic Project Template for Laravel applications, packages, APIs, and services. Laravel is a host framework and adapter surface; it must not become domain truth by default.
...
| `{{DOCUMENT_TITLE}}` | STRING | Title | Yes | Laravel Project |
...
| `{{DOCUMENT_TYPE}}` | STRING | Document Type | Yes | Laravel Project Specification |
...
| `{{PRODUCES}}` | LIST | Produces | Yes | Laravel Project document structure and placeholder manifest |
...
| `{{PROJECT_TYPE}}` | STRING | Not applicable | Yes | Laravel |
| `{{TARGET_PLATFORM}}` | STRING | Not applicable | Yes | PHP backend / web application |
| `{{HOST_FRAMEWORK}}` | STRING | Not applicable | Yes | Laravel |
| `{{RUNTIME_CONTEXT}}` | STRING | Not applicable | Yes | PHP / Laravel runtime |
| `{{LANGUAGE_STACK}}` | STRING | Not applicable | Yes | PHP |
...
| Target Platform | PHP backend / web application |
| Host Framework | Laravel |
| Runtime Context | PHP / Laravel runtime |
| Language Stack | PHP |
...
## 13. Laravel Extension

Laravel service providers, controllers, Eloquent models, migrations, events, jobs, queues, middleware, policies, and config files are implementation mechanisms unless project governance assigns explicit ownership.
