...
| Field | Value |
|:---|:---|
| Identifier | `FORGE-TPL-PROJECT-SYMFONY` |
| Title | Symfony Project Template |
| Version | `0.2.0-draft` |
| Status | Draft |
...
| Traceability ID | `FORGE-AI.TEMPLATES.PROJECT.SYMFONY` |
| Scope | Reusable output contract for symfony project adoption documentation. |
...
| Produces | Symfony Project document structure and placeholder manifest |
...
The Symfony Project Template extends the Generic Project Template for Symfony applications, bundles, backend services, and component-based PHP systems. Symfony is a host framework and adapter surface; it must not redefine framework authority or domain ownership.
...
| `{{DOCUMENT_TITLE}}` | STRING | Title | Yes | Symfony Project |
...
| `{{DOCUMENT_TYPE}}` | STRING | Document Type | Yes | Symfony Project Specification |
...
| `{{PRODUCES}}` | LIST | Produces | Yes | Symfony Project document structure and placeholder manifest |
...
| `{{PROJECT_TYPE}}` | STRING | Not applicable | Yes | Symfony |
| `{{TARGET_PLATFORM}}` | STRING | Not applicable | Yes | PHP backend / web application |
| `{{HOST_FRAMEWORK}}` | STRING | Not applicable | Yes | Symfony |
| `{{RUNTIME_CONTEXT}}` | STRING | Not applicable | Yes | PHP / Symfony runtime |
...
| Host Framework | Symfony |
| Runtime Context | PHP / Symfony runtime |
| Language Stack | PHP |
