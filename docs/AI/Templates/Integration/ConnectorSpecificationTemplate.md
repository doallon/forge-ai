...
| Field | Value |
|:---|:---|
| Identifier | `FORGE-TPL-INTEGRATION-CONNECTOR-SPECIFICATION` |
| Title | Connector Specification Template |
| Version | `2.0.0-draft` |
| Status | Draft |
...
| Traceability ID | `FORGE-AI.V2.TEMPLATES.INTEGRATION.CONNECTOR-SPECIFICATION` |
| Scope | Defines the reusable documentation structure for a connector that links Forge AI or a project to an external source, tool, service, or repository. |
...
| Produces | Reusable connector specification artifact structure |
...
Defines the reusable documentation structure for a connector that links Forge AI or a project to an external source, tool, service, or repository.
...
Use this template to produce a `Connector Specification` artifact when the active task explicitly needs that artifact type.
...
```
Default produced-document classification is `Integration Artifact` and default document type is `Connector Specification`.
...
| `{{CONNECTOR_NAME}}` | TEXT | Body content | Yes | Author-provided |
| `{{CONNECTOR_OWNER}}` | TEXT | Body content | Yes | Author-provided |
| `{{CONNECTOR_STATUS}}` | STATUS | Body content | Yes | Draft |
| `{{PURPOSE}}` | TEXT | Body content | Yes | Author-provided |
| `{{PROVIDES}}` | LIST | Body content | Yes | Author-provided |
| `{{CONSUMES_LIST}}` | LIST | Body content | Yes | Author-provided |
| `{{CAPABILITIES}}` | TABLE | Body content | Yes | Author-provided |
| `{{DATA_FLOW}}` | TEXT | Body content | Yes | Author-provided |
| `{{AUTHENTICATION_BOUNDARY}}` | TEXT | Body content | Yes | Author-provided |
| `{{AUTHORIZATION_BOUNDARY}}` | TEXT | Body content | Yes | Author-provided |
| `{{RATE_LIMITS_OR_CONSTRAINTS}}` | TEXT | Body content | Yes | Author-provided |
| `{{ERROR_HANDLING}}` | TEXT | Body content | Yes | Author-provided |
| `{{SECURITY_CONSIDERATIONS}}` | TEXT | Body content | Yes | Author-provided |
...
| Connector Owner | {{CONNECTOR_OWNER}} |
