...
| Produces | Reusable integration contract artifact structure |
...
Use this template to produce a `Integration Contract` artifact when the active task explicitly needs that artifact type.
...
```
Default produced-document classification is `Integration Artifact` and default document type is `Integration Contract`.
...
| `{{INTEGRATION_NAME}}` | TEXT | Body content | Yes | Author-provided |
| `{{INTEGRATION_OWNER}}` | TEXT | Body content | Yes | Author-provided |
| `{{RELATED_ADAPTER}}` | TEXT | Body content | Yes | Author-provided |
| `{{CONTRACT_STATUS}}` | STATUS | Body content | Yes | Draft |
| `{{PURPOSE}}` | TEXT | Body content | Yes | Author-provided |
| `{{INTEGRATION_SUMMARY}}` | TABLE | Body content | Yes | Author-provided |
| `{{CONTRACT_SURFACE}}` | TABLE | Body content | Yes | Author-provided |
| `{{ALLOWED_OPERATIONS}}` | LIST | Body content | Yes | Author-provided |
| `{{FORBIDDEN_OPERATIONS}}` | LIST | Body content | Yes | Author-provided |
| `{{SECURITY_AND_PERMISSIONS}}` | TABLE | Body content | Yes | Author-provided |
| `{{FAILURE_HANDLING}}` | TEXT | Body content | Yes | Author-provided |
...
| Related Adapter | {{RELATED_ADAPTER}} |
