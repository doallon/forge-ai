# Swarm Execution Report Template
...
| Field | Value |
|:---|:---|
| Identifier | `FORGE-TPL-SWARM-EXECUTION-REPORT` |
| Title | Swarm Execution Report Template |
| Version | `2.0.0-draft` |
| Status | Draft |
...
| Traceability ID | `FORGE-AI.V2.TEMPLATES.SWARM.EXECUTION-REPORT` |
| Scope | Defines the reusable documentation structure for reporting swarm summary, work-unit results, synchronization summary, merge summary, validation results, review readiness, risks, dissolution status, and recommended next step. |
...
| Produces | Reusable swarm execution report artifact structure |
...
Defines the reusable documentation structure for reporting swarm summary, work-unit results, synchronization summary, merge summary, validation results, review readiness, risks, dissolution status, and recommended next step.
...
Use this template to produce a `Swarm Execution Report` artifact when the active task explicitly needs that artifact type.
...
```
Default produced-document classification is `Swarm Artifact` and default document type is `Swarm Execution Report`.
...
| `{{REPORT_STATUS}}` | STATUS | Body content | Yes | Draft |
| `{{COORDINATOR}}` | TEXT | Body content | Yes | Author-provided |
| `{{REPORT_DATE}}` | DATE | Body content | Yes | Author-provided |
| `{{SUMMARY}}` | TEXT | Body content | Yes | Author-provided |
| `{{WORK_UNIT_RESULTS}}` | TABLE | Body content | Yes | Author-provided |
| `{{SYNCHRONIZATION_SUMMARY}}` | TEXT | Body content | Yes | Author-provided |
| `{{MERGE_SUMMARY}}` | TEXT | Body content | Yes | Author-provided |
| `{{VALIDATION_RESULTS}}` | TABLE | Body content | Yes | Author-provided |
| `{{REVIEW_READINESS}}` | CHECKLIST | Body content | Yes | Author-provided |
| `{{RISKS}}` | TABLE | Body content | Yes | Author-provided |
| `{{DISSOLUTION_STATUS}}` | CHECKLIST | Body content | Yes | Author-provided |
