...
| Produces | Reusable swarm review report artifact structure |
...
Defines the reusable documentation structure for reviewing completed swarm work for scope, ownership, validation, merge safety, certification readiness, findings, verdict, and recommended next step.
...
Use this template to produce a `Swarm Review Report` artifact when the active task explicitly needs that artifact type.
...
```
Default produced-document classification is `Swarm Artifact` and default document type is `Swarm Review Report`.
...
| `{{REVIEW_STATUS}}` | STATUS | Body content | Yes | Draft |
| `{{REVIEWER}}` | TEXT | Body content | Yes | Author-provided |
| `{{REVIEW_DATE}}` | DATE | Body content | Yes | Author-provided |
| `{{PURPOSE}}` | TEXT | Body content | Yes | Author-provided |
| `{{REVIEW_AREAS}}` | TABLE | Body content | Yes | Author-provided |
| `{{FINDINGS}}` | TABLE | Body content | Yes | Author-provided |
| `{{VERDICT}}` | TEXT | Body content | Yes | Author-provided |
| `{{CERTIFICATION_READINESS}}` | CHECKLIST | Body content | Yes | Author-provided |
