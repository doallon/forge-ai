...
| `VS-Q3` | Should Release/Package receive a new M.6 scope level, or resolve via composition? | `RC-01`, `RC-02` | Responsibility/ownership allocation (via `RC-01`/`RC-02`) | Gates B, D | **Unresolved** — `RC-01`/`RC-02` are now Decided (§10.9), but per the Decision Package's own rule that "a cross-reference is not a resolution of either mapped item," `VS-Q3` itself remains formally Unresolved pending separate recording |
...
| `NB-Q2` | Which §6 numbering-model family (or hybrid) for which entity class(es)? | `SV-01`; `VS-Q1` | Human Governance policy choice | Gate C | **Unresolved** — `NB-Q1`'s decision determines this selection is now made per entity class; it does not select a family |
...
| `NB-Q4` | Should Release/Package receive a new M.6 scope level, or resolve via composition? | `RC-01`, `RC-02`; `VS-Q3` | Responsibility/ownership allocation (via `RC-01`/`RC-02`) | Gates B, D | **Unresolved** — `RC-01`/`RC-02` are now Decided (§10.9); `NB-Q4` itself remains formally Unresolved pending separate recording, per the same cross-reference rule as `VS-Q3` |
...
Each item below carries **exactly one** primary decision class, as the Decision Package assigns it. No item is reassigned here. Three items — `NB-Q1`, `RC-01`, `RC-02` — are now Decided (§10.9); they remain listed in their original decision class below, annotated, since a decision does not change which class of question it was.

| Decision class | Items |
|:---|:---|
| **Human Governance policy choice** — a genuine value-laden choice among legitimate alternatives | `SV-01`/`VS-Q1`/`NB-Q2`; `SV-02`/`VS-Q2`/`NB-Q3`; `CP-06`/`VS-Q4`/`NB-Q5`; `NB-Q1` **(Decided — `NB-Q1-B`, §10.9)**; `VS-Q7`/`NB-Q9`; `VS-Q8`/`NB-Q10`; `NB-Q12`; `NB-Q13` (with `AU-01`/`VS-Q6`/`NB-Q8` and `NB-Q7` carried as HG ballot items per Decision Package §9) |
...
**[Established]** This draft **does not pass Gate B**, notwithstanding the decisions recorded at §10.9. The Decision Package's minimum set for Gate B (§10.2) was:
...
**Status as of 2026-07-28: this minimum set is recorded as cleared** — `NB-Q1` decided as `NB-Q1-B`, `RC-01` decided as `RC-01-D`, `RC-02` decided as `RC-02-E` (§10.9). **Clearing this minimum set does not itself complete Gate B.** Per the Decision Package's own Gate-Blocking Table (§6.1 there), a complete Gate-B candidate additionally requires at least explicit-dependency treatment of `CMP-01`, `CMP-03` (the "deployed"/"observed"-broad portions), `CMP-07`, `CMP-08` (the operational-completeness portion), and `CMP-09` (the "authorize operational reliance"/"observe deployment/use" portions), and further advancement of `NB-Q12` (only partially unblocked by the `RC-01`/`RC-02` decision, since its Tier-1 dependency on `NB-Q2` remains open). None of these is resolved by this recording.
`⚠ DEPENDENCY: CMP-01 unresolved` · `⚠ DEPENDENCY: CMP-03 (deployed/observed-broad) unresolved` · `⚠ DEPENDENCY: CMP-07 unresolved` · `⚠ DEPENDENCY: CMP-08 (operational-completeness) unresolved` · `⚠ DEPENDENCY: CMP-09 (authorize-operational-reliance/observe-deployment-use) unresolved` · `⚠ DEPENDENCY: NB-Q12 partially advanced only`

**[Established]** The Decision Package additionally states that `NB-Q1` and `RC-01`/`RC-02` belong to the Gate-B minimum set *"and **not** to the minimum set for Gate A — because their absence does not prevent drafting from beginning; it prevents a draft from being *completed* as internally coherent across every entity class and scope level STEP 8 examined"* (Decision Package §10.2). This remains true of the now-decided status: clearing the minimum set retroactively confirms Gate A was never blocked by these items, and does not advance Gate B beyond the minimum-set clearance just described.

**[Established]** This draft **does not pass Gate C.** Gate C is *"the Human Governance act of choosing among the synthesis candidate's presented alternatives"* (§11.1) — selecting a concrete numbering-model family, signaling family, or bump-rule family for adoption. That selection has not been made. As of 2026-07-28, Human Governance has decided exactly three Decision Package §9 ballot items — `NB-Q1` (`NB-Q1-B`), `RC-01` (`RC-01-D`), and `RC-02` (`RC-02-E`), per §10.9 — and no others. **`NB-Q1-B` establishes that entity-class-specific numbering-model and bump-rule families are authorized under a shared minimum comparison discipline; it selects no concrete family for any entity class.** `NB-Q2` (which family, for which entity class), `NB-Q3` (rebuttable-default acceptability), and `NB-Q13` (cross-family comparison) all remain Undecided, as does every other ballot item at Decision Package §9 not named above. This draft fills in no "HG Decision" or "Rationale" cell beyond the three now recorded at Decision Package §9/§9.1, and Gate C — the act of selecting a family — has not opened.
...
**[Established]** As of 2026-07-28, the Decision Package's own Gate-B minimum set is recorded as **cleared**: `NB-Q1` decided as `NB-Q1-B`; `RC-01`/`RC-02` decided as `RC-01-D`/`RC-02-E` (§10.9). **This does not complete Gate B.**

**[Established]** Completion of a candidate synthesis remains prevented by the following, none of which is resolved by the §10.9 decisions: the Coverage Assessment's Outcome D remains in force, and the following `CMP` items are recorded by the Decision Package §6.1 as still needing at least explicit-dependency treatment before a Gate-B candidate could be completed: `CMP-01`, `CMP-03` (the "deployed"/"observed"-broad portions), `CMP-07`, the operational-completeness portion of `CMP-08`, and the "authorize operational reliance" / "observe deployment/use" portions of `CMP-09`. `NB-Q12` is only partially advanced — its Tier-0b dependency on `RC-01`/`RC-02` is now cleared, but its Tier-1 dependency on `NB-Q2` (still Undecided) remains open.

### 12.3 Which Decisions Remain With Human Governance

**[Established]** `NB-Q1`, `RC-01`, and `RC-02` are no longer among the decisions remaining with Human Governance — they are Decided (§10.9). Every other item in §10.7's "Human Governance policy choice" class remains with Human Governance and is recorded "Undecided" in the Decision Package §9 ballot: `SV-01`/`VS-Q1`/`NB-Q2`; `SV-02`/`VS-Q2`/`NB-Q3`; `CP-06`/`VS-Q4`/`NB-Q5`; `VS-Q7`/`NB-Q9`; `VS-Q8`/`NB-Q10`; `NB-Q12`; `NB-Q13`; `AU-01`/`VS-Q6`/`NB-Q8`; `NB-Q7`.
...
| 4 | That Release has been classified at any semantic or artifact level, that an M.3 relationship mapping has been chosen, or that an M.6 Release/Package scope level exists or should exist. `RC-01` and `RC-02` are now Decided (§10.9) as **ownership allocations only** — naming a future artifact as owner of Release Composition and Release Identity policy, respectively — and decide none of the classification/mapping/scope-level questions this item lists; `VS-Q3` and `NB-Q4` remain formally open pending separate recording (§10.1, §10.2). |
...
| 13 | That any identifier in §10 other than `NB-Q1`, `RC-01`, and `RC-02` (Decided, §10.9) has been resolved, closed, merged with another, superseded, or reclassified. Nor that deciding `NB-Q1`/`RC-01`/`RC-02` resolves any *other*, merely cross-referenced identifier — `VS-Q3`, `NB-Q4`, `NB-Q2`, `NB-Q3`, `NB-Q13`, and the M.6 Release scope-level question all remain formally Unresolved, per the rule that a cross-reference is not a resolution of either mapped item (§10.3; Decision Package §4). |
...
It carries forward, under their source-native identifiers and without merging or dropping any, nine `VS-Q` items, fourteen `NB-Q` items, eighteen named reconciliation identifiers plus `CP-01`, and twelve `CMP` identifiers, each classified into exactly one decision class. As originally drafted, each was recorded as unresolved; as of 2026-07-28, three — `NB-Q1`, `RC-01`, `RC-02` — are recorded as Decided (§10.9), and every other identifier remains recorded as unresolved exactly as originally drafted.

As originally drafted, it selected no architecture family, resolved no open question, allocated no responsibility, amended no normative artifact, and converted no research evidence into normative truth. **This remains true with exactly the three named exceptions recorded at §10.9**, which reflect a Human Governance decision made after this draft was opened, not a change this draft makes to itself. It exercises **Gate A and only Gate A**, and — notwithstanding the §10.9 decisions clearing the Decision Package's Gate-B minimum decision set — it still does not pass Gate B, Gate C, Gate D, or Gate E. It is **Draft, non-normative, non-canonical, incomplete, and not a selected or approved Versioning Architecture**, and it is offered for Human Governance's consideration on exactly those terms.
